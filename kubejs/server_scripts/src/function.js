...
/**
 * 
 * @param {string} type timewarp, stamping, sieving
 * @param {$ItemStack_[]} inputs 
 * @param {$ItemStack_[]} outputs
 * @returns {$JsonObject_}
 */
function anvil_processing(type,inputs,outputs){
    //recipe type
    const json = createDefaultJsonObject("type", "anvilcraft:anvil_processing");
    //string properties
    addsToJsonObject(json,
        ["anvil_recipe_type"],
        [type]
    )
    //icon
    const icon = createJsonObject()
    let item = outputs[0]
    let id = item.id
    if(item instanceof $OutputItem){
        id = item.item.id
    }
    addToJsonObject(icon,"item",id)
    addToJsonObject(json,"icon",icon)
    //outcomes
    const outcomes = createJsonArray();
    //output
    outputs.forEach(output =>{
        //properties
        const outcome = createDefaultJsonObject("type", "spawn_item");
        let chance = 0;
        let count = output.getCount()
        let o = output
        let nbt = null
        if(output instanceof $OutputItem) {
            chance = output.hasChance() ? output.getChance() : chance
            o = output.item
        }
        //chance
        if(chance > 0){
            addToJsonObject(outcome,"chance",chance)
        }
        //nbt
        if(o.hasNBT()){
            nbt = o.getNbt()
        ...
        //offset
        const offset = createJsonArray()
        if(type == "timewarp"){
            addsToJsonArray(offset,[0.0,-1.0,0.0])
        }else if(type == " stamping"){
            addsToJsonArray(offset,[0.0,-0.75,0.0])
        }else{
            addsToJsonArray(offset,[0.0,-1.0,0.0])
        }
        addToJsonObject(outcome,"offset",offset)
        //result
        const result = createDefaultJsonObject("item",o.id)
        if(count > 1){
            addToJsonObject(result,"count",count)
        }
        if(nbt !== null){
            addToJsonObject(result,"nbt","" + nbt)
        }
        addToJsonObject(outcome,"result",result)
        addToJsonArray(outcomes,outcome)
    })
    addToJsonObject(json,"outcomes",outcomes)
    //predicates
    const predicates = createJsonArray()
    if(type == "timewarp"){
        let stateMap = new $HashMap()
        stateMap.put("lit",true)
        addToJsonArray(predicates,hasBlock("anvilcraft:corrupted_beacon",stateMap,[0.0,-2.0,0.0]))
        addToJsonArray(predicates,hasBlock("minecraft:cauldron",null,[0.0,-1.0,0.0]))
    }else if(type == "stamping"){
        addToJsonArray(predicates,hasBlock("anvilcraft:stamping_platform",null,[0.0,-1.0,0.0]))
    }else if(type == "sieving"){
        addToJsonArray(predicates,hasBlock("minecraft:scaffolding",null,[0.0,-1.0,0.0]))
    }
    inputs.forEach(item =>{
        addToJsonArray(predicates,hasIngredient(type,item))
    })
    addToJsonObject(json,"predicates",predicates)
    return json
}
 ...
 *
 * @param {string} id
 * @param {$HashMap_<string,*>} stateMap
 * @param {number[]} offset
 * @returns {$JsonObject_}
 */
function hasBlock(id,stateMap,offset){
    const pro = createDefaultJsonObject("type","has_block")
    //match block
    const match_block= createJsonObject()
    const blocks = createJsonArray()
    addToJsonArray(blocks,id)
    addToJsonObject(match_block,"blocks",blocks)
    if(stateMap !== undefined && stateMap !== null && !stateMap.isEmpty()){
        let state = createJsonObject()
        stateMap.forEach((key, value) => {
            addToJsonObject(state,key,value)
        })
        addToJsonObject(match_block,"state",state)
    }
    addToJsonObject(pro,"match_block",match_block)
    //offset
    const off_set = createJsonArray()
    addsToJsonArray(off_set,offset)
    addToJsonObject(pro,"offset",off_set)
    return pro
}
 ...
 * @returns {$JsonObject_}
 */
function hasIngredient(type,item){
    let count = item.getCount()
    let id = item.id
    const ing = createDefaultJsonObject("type","has_item_ingredient")
    //match item
    const match_item= createJsonObject()
    const items = createJsonArray()
    addToJsonArray(items,id)
    addToJsonObject(match_item,"items",items)
    const count_object = createDefaultJsonObject("min",count)
    addToJsonObject(match_item,"count",count_object)
    addToJsonObject(ing,"match_item",match_item)
    //offset
    const off_set = createJsonArray()
    if(type == "timewarp"){
        addsToJsonArray(off_set,[0.0,-1.0,0.0])
    }else if(type == "stamping"){
        addsToJsonArray(off_set,[0.0,-0.75,0.0])
    }else if(type == "sieving"){
        addsToJsonArray(off_set,[0.0,0.0,0.0])
    }else{
        addsToJsonArray(off_set,[0.0,-1.0,0.0])
    }
    addToJsonObject(ing,"offset",off_set)
    return ing
}
